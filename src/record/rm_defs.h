...
struct RmRecord {
    ...
    RmRecord &operator=(const RmRecord& other) {
        size = other.size;
        allocated_ = other.allocated_;
        if (other.data != nullptr && other.allocated_) {
            data = new char[size];
            memcpy(data, other.data, size);
        }
    ...
    // 重载操作符 <<
    friend std::ostream &operator<<(std::ostream &os, const RmRecord& rec) {
        os << rec.size << '\n';
        os.write(rec.data, rec.size);
        os << '\n';
        return os;
    }
    friend std::istream &operator>>(std::istream &is, RmRecord& rec) {
        is >> rec.size;
        rec.data = new char[rec.size];
        char ch;
        is.get(ch);
        is.read(rec.data, rec.size);
