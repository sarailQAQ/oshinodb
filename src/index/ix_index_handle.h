...
public:
    ...
    IxNodeHandle(const IxFileHdr *file_hdr_, Page *page_) : file_hdr(file_hdr_), page(page_) {
        page_hdr = reinterpret_cast<IxPageHdr *>(page->get_data() + Page::OFFSET_PAGE_HDR);
        keys = (char*)(page_hdr) + sizeof(IxPageHdr);
...
public:
    ...
    // for search
    bool get_value(const char *key, std::vector<Rid> *result, Context *context);

    IxNodeHandle *find_leaf_page(const char *key, Operation operation, Context *context,
                                                   bool find_first = false);

    // for insert
    page_id_t insert_entry(const char *key, const Rid &value, Context *context);

    IxNodeHandle *split(IxNodeHandle *node, Context *context);

    void insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node, Context *context);

    // for delete
    bool delete_entry(const char *key, Context *context);

    bool coalesce_or_redistribute(IxNodeHandle *node, Context *context, bool *root_is_latched = nullptr);

    bool adjust_root(IxNodeHandle *old_root_node, Context *context);

    void redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index, Context *context);
...
private:
    ...
    void maintain_parent(IxNodeHandle *node, Context *context);

    void erase_leaf(IxNodeHandle *leaf, Context *context);

    void release_node_handle(IxNodeHandle &node, Context *context);

    void maintain_child(IxNodeHandle *node, int child_idx, Context *context);

    void add_to_log(Context* context);

    void handle_dirty_page(Context *context, IxNodeHandle *page);
};