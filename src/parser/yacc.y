...
// keywords
%token SHOW TABLES CREATE TABLE DROP DESC INSERT INTO VALUES DELETE FROM ASC ORDER BY COUNT MAX MIN SUM AS LIMIT
...
%type <sv_orderbys_> order_clauses
%type <sv_orderby_dir> opt_asc_desc
%type <sv_func> aggregateFunc
%type <sv_select_col> selectCol
%type <sv_select_cols> selectColList selector
...
dml:
    ...
    |   SELECT selector FROM tableList optWhereClause order_clauses opt_limit
    {
...
order_clauses:
	...
	| /* epsilon */ {
		$$ = std::vector<std::shared_ptr<OrderBy> >(0);
	}
...
selectCol:
	aggregateFunc '(' '*' ')' AS colName
	{
		$$ = std::make_shared<SelectCol>($1, nullptr, $6);
	}
	| aggregateFunc '(' col ')' AS colName
        {
        	$$ = std::make_shared<SelectCol>($1, $3, $6);
        }
        | aggregateFunc '(' col ')'
        {
                $$ = std::make_shared<SelectCol>($1, $3);
        }
        | col
        {
         	$$ = std::make_shared<SelectCol>(SV_FUNC_NULL, $1);
        }
        ;
