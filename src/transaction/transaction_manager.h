...
public:
    explicit TransactionManager(LockManager *lock_manager, SmManager *sm_manager, BufferPoolManager* bpm,
...
private:
    static void add_idx_log(Transaction* txn, LogManager* log_manager) {
        lsn_t lsn = txn->get_prev_lsn();
        for (auto rec : txn->get_index_logs()) {
            rec.prev_lsn_ = lsn;
    ...
    void unpin_pages(Transaction* txn) {
        for (auto pair : txn->get_page_pins()) {
            auto pin_count = pair.second;
            auto& page_id = pair.first;
            while (pin_count-- && buffer_pool_manager_->unpin_page(page_id, true)) ;
