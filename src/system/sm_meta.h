...
struct TabMeta {
    ...
    bool is_index_prefix(const std::vector<std::string> &col_names,
                         std::vector<std::string> &index_col_names) const {
        size_t cur_size = 0;
        IndexMeta matched_index;
        for (auto &index: indexes) {
            int new_size = 0;
        ...
        if (cur_size > 0) {
            ...
            for (const auto& col : matched_index.cols) index_col_names.push_back(col.name);
            return true;
        }
        return false;
    }
