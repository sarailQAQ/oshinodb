        ...
        if (auto x = std::dynamic_pointer_cast<OtherPlan>(plan)) {
            ...
            switch (x->tag) {
                case T_select: {
                    ...
                    if (auto p = std::dynamic_pointer_cast<ProjectionPlan>(x->subplan_)) sel_cols_ = p->sel_cols_;
                    else if (auto p = std::dynamic_pointer_cast<AggregatePlan>(x->subplan_)) sel_cols_ = p->sel_cols_;
                    else throw InternalError("invalid select plan, expect aggregate or projection plan");

                    return std::make_shared<PortalStmt>(PORTAL_ONE_SELECT, std::move(sel_cols_), std::move(root),
                                                        plan);
                }

                case T_Update: {
//                    if (!context->lock_mgr_->lock_exclusive_on_table(context->txn_, x->tab_name_))
...
//                        throw TransactionAbortException(context->txn_->get_transaction_id(), AbortReason::DEADLOCK_PREVENTION);
    ...
    void run(std::shared_ptr<PortalStmt> portal, QlManager *ql, txn_id_t *txn_id, Context *context) {
        switch (portal->tag) {
            ...
            case PORTAL_DML_WITHOUT_SELECT: {
                ql->run_dml(std::move(portal->root), context);
                break;
            }
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context) {
        if (auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)) {
            std::cerr << "proj executor" << std::endl;
            ...
            std::cerr << "sort executor" << std::endl;
            return std::make_unique<SortExecutor>(convert_plan_executor(x->subplan_, context),x->key_cols_, x->limit);
        }
        return nullptr;
    }
